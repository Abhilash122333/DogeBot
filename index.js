...
//// Image ///
const time2 = moment().tz("Asia/India").format("HH:mm:ss");
    if (time2 < "24:59:00") {
...
const checkRegisteredUser = (sender) => {
    ...
    const ownerNumber = [`918260349917@s.whatsapp.net`]
...
groups = xeon.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "918260349917@s.whatsapp.net"
        dtod = '918260349917'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
case 'bugreport':
              ...
              xeon.sendMessage('918260349917@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
case 'script':
		case 'sc':
		case 'sourcecode':
		xeon.sendMessage(from, { text: "Bot is private", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
